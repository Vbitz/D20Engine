...
class Shlexer {
  ...
  constructor(value: string) {
    ...
    /**
     * Characters that will be considered as escape. Just
     * `\` by default.
     */
    this.escapes = '\\';

    /**
     * The subset of quote types that allow escaped characters. Just
     * `"` by default.
     */
    this.escapedQuotes = '"';
  ...
  * [Symbol.iterator]() {
    ...
    let escaped: string|boolean = false;
    let token;
    ...
    while (true) {
      ...
      if (this.debug) {
        console.log(
            'input:', '>' + char + '<', 'accumulated:', token,
            'inQuote:', inQuote, 'escaped:', escaped);
      }
      ...
      if (escaped) {
        if (char === '\n') {
          ...
          // line. We just need to ignore it.
            ...
            !inQuote || char === inQuote || this.escapes.includes(char)) {
          token = (token || '') + char;
        } else {
